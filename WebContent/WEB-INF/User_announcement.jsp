...
<c:if test="${!empty factor}">   <label style="padding: 15px;">Edit your criteria</label>
					...
					<input type="text" id="factor_higher_price"
						...
						maxlength="60" value="${fn:trim(factor[1])}"></input><input type="text"
...
<c:if test="${empty factor}"> 
 <p class="erreur">
Please register your criteria.</p>
</c:if>
<c:if test="${!empty factor}"> 
    <c:choose>
            <c:when test="${ empty annoucement_user_factor }">
                <p class="erreur">
No annonce correspond to your criteria.</p>
            </c:when>
            <c:otherwise>
		<div>
			...
			<table>
				...
				<c:forEach var="annoucement" items="${annoucement_user_factor}" varStatus="status">
					<tr>
						<form action="announcement" method="post">
