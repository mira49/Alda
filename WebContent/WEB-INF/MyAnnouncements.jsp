	...
	<div id="corpsDa">
		...
		<c:choose>
			<c:when test="${ empty annoucement_user }">
				...
				<a style="float: right" href="<c:url value="/add_Announcement"/>"><img
					src="<c:url value="/inc/add.png"/>" />
			...
			<c:otherwise>
				...
				<table>
					...
					<form action="myAnnouncements" method="post"
						...
						<c:forEach var="annoucement" items="${ annoucement_user}">
							<tr>
								...
								<td><input type="image" name="view"
									...
									src="<c:url value="/inc/loupe.png"/>" alt="View" /> <input
									type="image" name="delete" value="${annoucement.id}"
									onclick="document.announcement_Form.submit();"
									src="<c:url value="/inc/delete.png"/>" alt="Supprimer" /> <input
									type="image" name="edit" value="${annoucement.id}"
									onclick="document.announcement_Form.submit();"
									src="<c:url value="/inc/edit.png"/>" alt="Edit" /></td>

								<c:if test="${annoucement.sold=='0'}">
									<td><button type="submit" name="sold"
											value="${annoucement.id}" class="sansLabel">Sold</button></td>
								</c:if>
