...
<body>
	...
	<div id="corpsSa">
		...
		<form action="homeBuyer" method="post" name="announcement_Form">
			<label style="padding: 15px;">Filter</label> <input type="text"
				...
				placeholder="Surface lower" maxlength="60"
				value="${fn:trim(factor[3])}"></input> <input type="text"
				id="factor_higher_surface" name="factor_higher_surface" size="20"
				placeholder="Surface higher" maxlength="60"
				value="${fn:trim(factor[4])}"></input> <input type="text"
	...
	<c:choose>
		<c:when test="${ empty annoucement_user }">
			<p class="erreur">No annonce register.</p>
		</c:when>
		<c:otherwise>
			<div id="corpsSam">
					...
					<tr>
						...
						<form method="get" action="contactBuyer" id="contact_Form">
							<td><button type="submit" class="btn btn-link "
									name="contact" value="${annoucement.id}">Contact</button> <input
								type="image" style="float: right; margin-top: 7px;" name="view"
